...
import com.mobilesafe.engine.AppInfos;
...
private Handler handler=new Handler(new Handler.Callback() {
    ...
    private void initUI() {
        ...
        //获取room内存的运行的剩余空间
        long room_freeSpace=Environment.getDataDirectory().getFreeSpace();
        //获取sd卡剩余的空间
       long sd_freeSpace=Environment.getExternalStorageDirectory().getFreeSpace();
        System.out.println("内存可用：" + room_freeSpace);
        System.out.println("sd卡可用：" + sd_freeSpace);
        //格式化大小
        tv_room.setText("内存可用:"+Formatter.formatFileSize(this,room_freeSpace));
        tv_sd.setText("sd卡可用:" + Formatter.formatFileSize(this, sd_freeSpace));

        UnstallReceiver receiver=new UnstallReceiver();
            ...
            public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
                ...
                if (userAppInfos != null && systemAppInfos != null) {
                    if (firstVisibleItem > (userAppInfos.size() + 1)) {
                        //系统应用程序
                        tv_app.setText("系统应用程序(" + systemAppInfos.size() + ")个");


                    } else {
                        tv_app.setText("用户应用程序(" + userAppInfos.size() + ")个");
...
//                System.out.println("totalItemCount==="+totalItemCount);
        ...
        list_view.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (obj != null && obj instanceof AppInfo) {
                    ...
                    //需要注意，使用PopupWindow 一定要设置背景，不然没有ScaleAnimation 动画
                    popupWindow.setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));
                    int[] location = new int[2];
                    //获取view展示到窗体上面的位置
                    view.getLocationInWindow(location);
                    popupWindow.showAtLocation(parent, Gravity.LEFT + Gravity.TOP, 165, location[1]);
                    //缩放的动画
    ...
    private  class  UnstallReceiver extends BroadcastReceiver{
        @Override
